...
export async function POST(req: Request) {
    try {
        ...
        const extensionSlug = (body.extensionSlug || body.slug || body.Slug || "").toLowerCase().trim();

        if (!email || !extensionSlug) {
            return NextResponse.json(
                { error: "Missing required fields: email and extensionSlug", received: { email: !!email, slug: !!extensionSlug } },
                { status: 400 }
            );
        }
